...
//! time the userspace changed. We now ship the rootserver inside the
//! Simpleboot initrd module (a USTAR tar archive). The kernel parses
//! the initrd at runtime via `crate::initrd::find_file()`.
//!
...
/// The rootserver's ELF64 image, located inside the Simpleboot initrd.
