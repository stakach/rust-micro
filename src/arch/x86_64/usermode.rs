...
const SENDER_PAYLOAD: &[u8] = &[
    0x48, 0xC7, 0xC2, 0x4D, 0x00, 0x00, 0x00, // mov rdx, 'M'
...
const RECEIVER_PAYLOAD: &[u8] = &[
    ...
    0x0F, 0x05,                               // syscall
    // After SysRecv, the message is in our TCB's msg_regs[0..3]
    // which the kernel fans out to user via the saved
    // user_context. The handler currently doesn't expose that to
    // user space (Phase 14e wires up MessageInfo return), so we
    // approximate by putchar'ing a known-good 'M' so the test
    // observes both bytes.
    0x48, 0xC7, 0xC7, 0x4D, 0x00, 0x00, 0x00, // mov rdi, 'M'
    0x48, 0xC7, 0xC0, 0xF7, 0xFF, 0xFF, 0xFF, // mov rax, -9
    0x0F, 0x05,                               // syscall
    0xEB, 0xFE,                               // jmp $-0
];
