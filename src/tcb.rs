...
pub struct Tcb {
    ...
    /// SC pool index this thread has an outstanding
    /// `SchedContext_YieldTo` against. seL4's `tcbYieldTo`. The
    /// consumed-report syscall return is written lazily when this
    /// thread is next dispatched (upstream activateThread →
    /// schedContext_completeYieldTo).
    pub yield_to: Option<u16>,
    ...
    /// Carries the `can_grant` bit from the sender's cap into the
    /// queued state, so a late-arriving receiver knows whether to
