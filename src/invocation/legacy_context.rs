...
use super::legacy_context_protocol::{FX_MASK, READ_WORDS, REGISTER_WORDS, WRITE_WORDS};
    ...
    unsafe {
        ...
        if write {
            ...
            if selection & FX_MASK != 0 {
                for (bytes, word) in fx.0.chunks_exact_mut(8).zip(&words[1 + REGISTER_WORDS..]) {
        ...
        // hardware owners are flushed no CPU can run the target between the two state groups.
        ...
        if write {
            ...
            for (word, bytes) in words[REGISTER_WORDS..READ_WORDS]
                .iter_mut()
