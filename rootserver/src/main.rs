...
/// MCS labels. These MUST match the kernel's generated `InvocationLabel`
/// enum (codegen out/invocations.rs). They were renumbered when the invocation
/// set was aligned to upstream; the old 33/34 silently misrouted (the kernel
/// saw an unrelated label and no-op'd), so SchedControl/SchedContext
/// invocations did nothing — a bound-SC child never became schedulable.
const LBL_SCHED_CONTROL_CONFIGURE: u64 = 37;
const LBL_SCHED_CONTEXT_BIND: u64 = 38;
const LBL_TCB_SET_PRIORITY: u64 = 6;
/// IRQ-handler invocations (kernel enum: IRQIssueIRQHandler=30, IRQSetIRQHandler=32).
const LBL_IRQ_ISSUE_IRQ_HANDLER: u64 = 30;
const LBL_IRQ_SET_IRQ_HANDLER: u64 = 32;
...
/// kernel).
// Must match the kernel's generated InvocationLabel enum (out/invocations.rs):
// X86PDPTMap=43, X86PageDirectoryMap=45, X86PageTableMap=47, X86PageMap=51,
// X86PageUnmap=52. The old 39/41/43/45/46 misrouted every map/unmap (silently,
// since rax is preserved) → the vspace demo's page maps never happened.
...
/// Phase 32g — first per-CPU SchedControl cap (core 0). Slots 16..19
/// hold the per-CPU SchedControl caps — a fixed boot-cap region,
/// unaffected by how far the empty region starts.
